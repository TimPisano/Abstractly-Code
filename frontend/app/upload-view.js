...
const Upload = {
    ...
    //
    // Checked first, and mutually exclusive with the not-a-lease
    // warning below: 'ocr_needed' means extraction never ran at all
    // (see document_extractor.find_low_text_pages), so "no tenant/
    // landlord/rent/dates were found" would be actively misleading --
    // nothing was actually searched for.
    nonLeaseWarningHtml(lease) {
        if (lease.processing_status === 'ocr_needed') {
            return `
                <div class="upload-result-warning">
                    ...
                    <span>${escapeHtml(lease.processing_error || 'This document needs OCR -- one or more pages have no usable text layer, so extraction was skipped.')}</span>
                </div>
            `;
        }
